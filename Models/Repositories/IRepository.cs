...
namespace UVSITCU.Models.Repositories
{
  ...
  public class OvertimeRepository : TRepository<Overtime>
  {
    public OvertimeRepository(string conString) : base(conString, "Overtimes") { }


    public override async Task<bool> Post(Overtime obj)
    {
      string query = @"update Overtimes set 
                                UserId=@UserId, 
                                Date=@Date,
                                Minutes=@Minutes,
                                UsedDate=@UsedDate,
                                AppliedPersonId=@AppliedPersonId 
                            where Id=@Id";
    ...
    public override async Task<int> Put(Overtime obj)
    {
      var query = @"insert into Overtimes (UserId, Date, Minutes)
                            output inserted.Id
